...
Retrieval-Augmented Generation (RAG) techniques blend the generative capabilities of LLMs with the retrieval of information from diverse knowledge bases (KBs). 
...
sequenceDiagram title Traditional RAG
    ...
    Note over Service,KB: Naive Knowledge Retrieval

    Service->>+LLM: What is the meaning of life (context: [42, Coffee])
    LLM-->>-Service: 42
    Note over Service,LLM: Augmented Generation
...
sequenceDiagram title RAG Tool
    ...
    Note over Service,LLM: LLM Selects Tools

    Service->>+KB: meaning of life
    KB-->-Service: [42, Coffee]
    Note over Service,KB: LLM-Directed Retrieval
...
We'll use Vercel's Generative UI library to render the chat window and Dewy to implement the information storage and retrieval used by our tool. 

## Why Dewy and Vercel?

**Vercel's [Generative UI library](https://sdk.vercel.ai/docs/concepts/ai-rsc)** is designed to simplify the creation of custom LLM "tools" and to render rich, interactive UI elements that are specific to each tool the LLM chooses to use. 
What this means in practice is a seamless and engaging user experience, where the interface adapts and evolves based on the context of the query and the tools selected by the LLM. 

**[Dewy](https://github.com/DewyKB/dewy)** is an OSS knowledge base designed to streamline the way developers store, organize, and retrieve information. 
Dewy is a knowledge base with the simplicity and ease-of-use of a document store:  insert your documents and Dewy takes care of preparing them for semantic searching. Dewy's flexibility and ease of use make it an excellent choice for developers aiming to build custom knowledge-driven LLM tools.
...
* A copy of Dewy running on your local machine (see Dewy's [installation instructions](http://localhost:3001/docs/Getting%20Started/installation) if you need help here).
...
To keep this post from going any longer than it is already, several basic UI components have been omitted, but you can checkout the full implementation in the [examples directory](https://github.com/DewyKB/dewy-ts/tree/main/examples) of the Dewy repo.
