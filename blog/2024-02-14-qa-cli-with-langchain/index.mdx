...
export async function load(collection: string, url: string, options: { collection: string, dewy_endpoint: string }): Promise<void> {
  ...
  try {
    ...
    const result = await dewy.kb.addDocument({ collection, url });
...
```sh   
npm install dewy_langchain langchain @langchain/openai openai
```
